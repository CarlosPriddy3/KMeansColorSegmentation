...
public class Operations 
{
        ...
        for (int i = 0; i < imageHeight; i++)
        {
            for(int j = 0; j < imageWidth; j++)
            {
                ...
                else
                {
	                for(int m = 0; m < k; m++)
	                {
	                    findLowestDistance[m] = (float)((distanceArray[m]*0.3) + (distanceArrayNeighbourhood[m]*0.7));
	                }
                }
    ...
    public static BufferedImage outputImage(RandomCentroid[] centroidList, PixelObjects[] pixelObjectArray)
    {
        ...
        System.out.println("Please enter a name for the file to be saved including extension: ");
        ...
        {
            if (centroidList[i].getIsActive() == 0)
            {
                System.out.println("Centroid #" + i + " was not used due to insufficient pixels");
            }
            else
                System.out.println("Centroid #" + i + " was used!");
        }*/
    }
